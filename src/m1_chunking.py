    ...
    for parent in parents:
        ...
        for cp in child_paras:
            if len(current_child_text) + len(cp) > child_size and current_child_text:
                children.append(Chunk(text=current_child_text.strip(), metadata={**metadata, "chunk_type": "child", "chunk_index": len(children)}, parent_id=pid))
                current_child_text = ""
    ...
    for section in sections:
        if re.match(r'^#{1,3}\s+', section):
            if current_content.strip():
                text_content = f"{current_header}\n\n{current_content.strip()}" if current_header != "General" else current_content.strip()
                chunks.append(Chunk(
                    text=text_content,
